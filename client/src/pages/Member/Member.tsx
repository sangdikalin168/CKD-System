...
export const Member = () => {
    ...
    const { data, loading: member_loading, refetch } = useGetCustomersQuery();
        ...
        <>
            {
                !member_loading ? (
                    <div>
                        {showCreate && <CreateMember setShow={setShowCreate} onSubmit={handleSubmit} />}
                        {showProfile && <MemberProfile ID={customer_id} />}
                        {!showProfile && <DataTable columns={columns} data={data_table} button={add_button} />}
                    </div>
