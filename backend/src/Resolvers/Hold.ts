...
export class HoldResolver {
    ...
    async GetHoldRequest(@Arg("customer_id") customer_id: number) {
        const result = await HoldRequest.query("SELECT * FROM hold_request WHERE customer_id = ? ORDER BY request_id DESC LIMIT 1", [customer_id])
    ...
    async CreateHold(
        ...
        if (res.hold_id) {
            ...
            if (update.affected) {
                // Dual-write: push the membership entitlement's end out to match.
                try {
                    // Revives/extends the membership line even if it had expired.
                    await extendMembershipEntitlement(customer_id, new_end);
                } catch (e) {
                    console.error("Unified-model dual-write failed (hold):", e);
                }
