...
import { extendMembershipEntitlement } from "../Utils/access";
import { Arg, Field, InterfaceType, Mutation, ObjectType, Query, Resolver, UseMiddleware } from "type-graphql";
    ...
    async CreateTransfer(
        ...
        if (res.transfer_id) {
            ...
            if (update_sender.affected) {
                ...
                // Dual-write: move the time on the membership entitlements too.
                try {
                    // Both sides revive/extend even if their membership had expired.
                    await extendMembershipEntitlement(sender_id, sender_new_end);
                    await extendMembershipEntitlement(receiver_id, receiver_new_end, receiver_old_end);
                } catch (e) {
                    console.error("Unified-model dual-write failed (transfer):", e);
                }
